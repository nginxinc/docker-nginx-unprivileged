...
IMAGE_DIGESTS=$(aws ecr-public describe-images \
  ...
  --query 'imageDetails[?!imageTags && (contains(imageManifestMediaType, `manifest.list.v2`) || contains(imageManifestMediaType, `image.index.v1`)) && `date -d @{imagePushedAt} +%s` < `'$CUTOFF_DATE'`].{imageDigest: join(`=`, [`imageDigest`, imageDigest])}' \
  --output text)
...
IMAGE_DIGESTS=$(aws ecr-public describe-images \
  --repository-name "${REPOSITORY_NAME}" \
  --query 'imageDetails[?!imageTags && `date -d @{imagePushedAt} +%s` < `'$CUTOFF_DATE'`].{imageDigest: join(`=`, [`imageDigest`, imageDigest])}' \
  --output text)
